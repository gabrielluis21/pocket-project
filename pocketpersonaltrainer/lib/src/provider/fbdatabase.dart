...
class FbDatabase {
  ...
  Future<List<UserModel>> getAllUser() async {
    try {
      ...
      var snaps = _database.collection("users").snapshots();
      snaps.forEach((action) {
        for (var element in action.docs) {
  ...
  updateUserExercise(UserExercises obj, String userId) async {
    try {
      await _database.collection("users").doc(userId).collection("myExercises").doc(obj.id).update(obj.toMap());
    } catch (e) {
      print(e);
      return e;
    }
