...
class _HomePageState extends State<HomePage> with PremiumFeaturesMixin {
  ...
  Widget buildHomeView() {
    return Stack(
      ...
      children: [
        ...
        Visibility(
          visible: purchaseController.isPremium.value == false ? true : false,
          child: Positioned(
            bottom: 0,
  ...
  Widget buildPageView() {
    return PageView(
      ...
      physics: bottomSelectedIndex == 3 ? NeverScrollableScrollPhysics() : null,
      onPageChanged: (index) {
        pageChanged(index);
      },
      children: [
  ...
  void initState() {
    ...
    purchaseController.isPremium.value ? Get.find<GymController>(tag: 'gyms').loadAllForPremiumUsers() : Get.find<GymController>(tag: 'gyms').loadAll();
    super.initState();
  }
