...
class ExerciseScreen extends StatefulWidget {
  ...
  const ExerciseScreen({Key? key, required this.exercise, required this.language}) : super(key: key);
...
class _ExerciseScreenState extends State<ExerciseScreen> with SingleTickerProviderStateMixin {
  ...
  List<Widget> buildImageList() {
    return List<Widget>.from(
      growable: false,
      widget.exercise.images?[widget.language]?.map(
        (e) => GestureDetector(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      /* decoration: BoxDecoration(
        : settingsController.themeMode == ThemeMode.dark ? AppColorTheme().customDarkColorScheme.surface : AppColorTheme().customLightColorScheme.surface,
      ), */
      child: Scaffold(
  ...
  Future<void> addExerciseToUserList() async {
    ...
    print(stringToDate);
    userExercise.dateMarked = DateTime.parse('${stringToDate[2]}-${stringToDate[1]}-${stringToDate[0]} 00:00:00.00000');
